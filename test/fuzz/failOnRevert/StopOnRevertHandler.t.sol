...
import {MockV3Aggregator} from "../../mocks/MockV3Aggregator.sol"; //to update priceFeed answers
...
contract StopOnRevertHandler is Test {
    ...
    function redeemCollateral(uint256 collateralSeed, uint256 amountCollateral) public {
        ERC20Mock collateral = _getCollateralFromSeed(collateralSeed);
        //only allow to redeem max amount of DSC in system
        uint256 maxCollateral = dscEngine.getCollateralBalanceOfUser(msg.sender, address(collateral));
        // we will bound amountCollateral to max amount.
